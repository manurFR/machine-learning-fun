...
from linalg import get_column, shape, make_matrix, magnitude, dot, vector_sum, scalar_multiply, vector_substract
...
def remove_projection_from_vector(vector, direction_vector):
    return vector_substract(vector, project(vector, direction_vector))


def remove_projection(matrix, direction_vector):
    return [remove_projection_from_vector(row, direction_vector) for row in matrix]
...
def principal_component_analysis(matrix, num_components):
    ...
    for _ in range(num_components):
        ...
        matrix = remove_projection(matrix, component)
    return components
...
def transform(matrix, components):
    return [transform_vector(row, components) for row in matrix]
